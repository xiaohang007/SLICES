...
def train_model(run_name, batch_size, max_epochs, n_embd, n_layer, n_head, lr, n_visible, *prop_cols):
    ...
    try:
        ...
        if process.returncode != 0:
            # 这里是错误的地方: stderr已经重定向到stdout，不能再单独读取
            # 改为直接报告进程返回值
            log_output += f"\n进程以非零退出码结束: {process.returncode}"
...
def generate_structures(model_weight, prop_targets, gen_size, batch_size):
    ...
    try:
        ...
        train_dir = os.path.join(os.getcwd(), "1_train_generate")
        # Load config file
        config_path = os.path.join(model_dir, f"{os.path.splitext(model_weight)[0]}.json")
        vocab_path = os.path.join(model_dir, f"{os.path.splitext(model_weight)[0]}_vocab.json")
...
def check_novelty(run_name, structure_json):
    ...
    try:
        ...
        error_msg = f"新颖性检查过程中发生异常: {str(e)}\n"
        error_msg += f"详细错误信息: {traceback.format_exc()}"
        yield error_msg, None  
...
def load_model_info(model_weight):
    ...
    try:
        ...
        # 读取prop_cols
        prop_json_path = os.path.join(model_dir, f"{os.path.splitext(model_weight)[0]}.json")
        if os.path.exists(prop_json_path):
            with open(prop_json_path, 'r') as f:
                prop_config = json.load(f)
                prop_cols = prop_config.get("prop_cols", [])
        else:
            # 从训练数据集中尝试获取属性列
            dataset_dir = os.path.join(os.getcwd(), "0_dataset")
            train_df = pd.read_csv(os.path.join(dataset_dir, "train_data.csv"))
            prop_cols = [col for col in train_df.columns if col != "SLICES" and col != "crystal_system"]
    ...
    with train_generate_tab:
        ...
        with gr.Tab("生成") as generate_tab:
            ...
            load_model_btn.click(
                load_model_info,
                inputs=[model_weight],
                outputs=[prop_guide, prop_targets]
            )
