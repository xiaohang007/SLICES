...
def sample_smiles(voc_dir, nums, energy, outfn,tf_dir, until=False):
    ...
    if not until:
        ...
        seqs, likelihood, _ = transfer_model.sample(int(nums), float(energy))
        valid = 0
        double_br = 0
        ...
        n_sample = 0
        while valid < nums:
            seq, likelihood, _ = transfer_model.sample(1,torch.tensor(float(energy)))
            n_sample += 1
            #seq = seq.cpu().numpy()
            #seq = seq[0]
            # print(seq)
