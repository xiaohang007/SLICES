...
with open('../3_fix_valid_check/results_3_fix_valid_check.sli', 'r') as f:
    cifs=f.readlines()
cifs_split=list(split_list(cifs[:1000],threads))

for i in range(len(cifs_split)):
    os.mkdir('job_'+str(i))
    os.system('cp -r ./workflow/. job_'+str(i))
    with open('temp.sci', 'w') as f:
        f.writelines(cifs_split[i])
    os.system('mv temp.sci job_'+str(i))
