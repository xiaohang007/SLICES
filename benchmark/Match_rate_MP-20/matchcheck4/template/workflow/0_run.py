...
for i  in range(len(cifs)):
    ...
    try:
        ...
        structures,energy=CG.to_4structures(bond_scaling, delta_theta, delta_x, \
        lattice_shrink,lattice_expand,angle_weight,vbond_param_ave_covered,vbond_param_ave,repul)
        if len(structures)==4:
            a,b,c,d,e,f,g,h=CG.match_check4(ori,structures[3],structures[2],structures[1],structures[0])
            a2,b2,c2,d2,e2,f2,g2,h2=CG.match_check4(ori,structures[3],structures[2],structures[1],structures[0],ltol=0.3, stol=0.5, angle_tol=10)
            time_used=(time.time() - start_time)
            with open("result.csv",'a') as fn:
                fn.write(cifs[i]["material_id"]+','+a+','+b+','+c+','+d+','+a2+','+b2+','+c2+','+d2+','+e2+','+f2+','+g2+','+h2+','+str(num_atoms)+','+str(time_used)+'\n')
        if len(structures)==3:
            ...
            time_used=(time.time() - start_time)
            with open("result.csv",'a') as fn:
                fn.write(cifs[i]["material_id"]+','+a+','+b+','+'0'+','+c+','+a2+','+b2+','+'0'+','+c2+','+d2+','+e2+','+'1'+','+f2+','+str(num_atoms)+','+str(time_used)+'\n')
        if len(structures)==2:
            a,b,c,d=CG.match_check(ori,structures[1],structures[0],ltol=0.2, stol=0.3, angle_tol=5)
            a2,b2,c2,d2=CG.match_check(ori,structures[1],structures[0])
            time_used=(time.time() - start_time)
            with open("result.csv",'a') as fn:
                fn.write(cifs[i]["material_id"]+','+'0'+','+a+','+'0'+','+b+','+'0'+','+a2+','+'0'+','+b2+','+'1'+','+c2+','+'1'+','+d2+','+str(num_atoms)+','+str(time_used)+'\n')
