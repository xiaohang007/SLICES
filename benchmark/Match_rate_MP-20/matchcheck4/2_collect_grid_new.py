...
with open("results_collection_"+pwd.split("/")[-1]+".csv",'w')as f:
    f.write('folder,opt2_match_sum,opt_match_sum,std2_match_sum,std_match_sum,opt2_match2_sum,opt_match2_sum,std2_match2_sum,std_match2_sum,opt2_topo2_sum,opt_topo2_sum,std2_topo2_sum,std_topo2_sum,time_sum, \
...
for i in glob.glob("job_*/result*.csv"):
    ...
    data = pd.read_csv(i,dtype={'opt2_match': np.int32,'opt_match': np.int32, 'std2_match': np.int32,'std_match': np.int32,'opt2_match2': np.int32,'opt_match2': np.int32, 'std2_match2': np.int32,'std_match2': np.int32,'opt2_topo2': np.float64, 'opt_topo2': np.float64,'std2_topo2':np.float64,'std_topo2':np.float64, 'time':np.float64})
...
#        r.write(i.split('/')[0]+','+str(pd.DataFrame(data['recreated_match']).sum(numeric_only=True))+'\n')
    ...
    with open("results_collection_"+pwd.split("/")[-1]+".csv",'a')as r:
        r.write(i.split('/')[0]+','+str(data['opt2_match'].sum()/num*100)+','+str(data['opt_match'].sum()/num*100)+','+str(data['std2_match'].sum()/num*100)+','+str(data['std_match'].sum()/num*100)+ \
        ','+str(data['opt2_match2'].sum()/num*100)+','+str(data['opt_match2'].sum()/num*100)+','+str(data['std2_match2'].sum()/num*100)+','+str(data['std_match2'].sum()/num*100)+ \
        ','+str(data['opt2_topo2'].sum()/num)+','+str(data['opt_topo2'].sum()/num)+','+str(data['std2_topo2'].sum()/num)+','+str(data['std_topo2'].sum()/num)+','+str(data['time'].sum()/32/3600)+','+ \
