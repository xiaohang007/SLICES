...
with open('../0_get_json_mp_api/cifs_filtered.json', 'r') as f:
    cifs=json.load(f)
num=len(cifs)
print(num)
import configparser
...
#        r.write(i.split('/')[0]+','+str(pd.DataFrame(data['recreated_match']).sum(numeric_only=True))+'\n')
    ...
    with open("results_collection_"+pwd.split("/")[-1]+".csv",'a')as r:
        ...
        ','+str(data['opt2_topo2'].sum()/num)+','+str(data['opt_topo2'].sum()/num)+','+str(data['std_topo2'].sum()/num)+','+str(data['time'].sum()/80/3600)+','+ \
