...
def get_multiple_predictions_list(
    atoms_array=[],
    cutoff=8,
    neighbor_strategy="k-nearest",
    max_neighbors=12,
...
with open("output.csv") as f:
    ...
    for row in reader:
        ...
        with open(temp_dir.name+'/temp.vasp','w') as f1:
            f1.write('\n'.join(row[4].split('\\n')))
        atoms=Atoms.from_poscar(temp_dir.name+'/temp.vasp')
        print(get_prediction(atoms=atoms,model_name="jv_mbj_bandgap_alignn"))
        temp_dir.cleanup()
...
predictions_list=get_multiple_predictions_list(atoms_array=atoms_array,model_name="jv_optb88vdw_bandgap_alignn")


if len(predictions_list)==len(input_array):
    with open("result.csv", 'w') as f:
        for i in range(len(predictions_list)):
            f.write(','.join(input_array[i])+','+str(predictions_list[i])+'\n')
else:
    print("error")
