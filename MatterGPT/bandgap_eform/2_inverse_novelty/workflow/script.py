...
check=False
CG=SLICES(graph_method=graph_method, check_results=check, relax_model="chgnet")
results=[]
with open('temp_splited.csv', 'r') as f:
    reader = csv.reader(f)
    for row in reader:
        try:
            if CG.check_SLICES(row[2],strategy=4,dupli_check=False):
                CG.from_SLICES(row[2],strategy=4,fix_duplicate_edge=True)
                ...
                with open("result.csv",'a') as fn:
                    fn.write(row[0]+','+row[1]+','+row[2] +','+primitive_standard_structure.to(fmt="poscar").replace('\n','\\n')+','+str(novelty)+'\n')
        except Exception as e:
            print(e)
