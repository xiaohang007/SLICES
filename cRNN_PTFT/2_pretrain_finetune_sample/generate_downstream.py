...
def sample_slices(voc_dir, gen_size,batch_size, energy, outfn,tf_dir,append=False, scaler_type=0):
    """Sample smiles using the transferred model"""
    voc = Vocabulary(init_from_file=voc_dir)
    transfer_model = RNNHead(voc)
    if append:
        output = open(outfn, 'a')
    else:
        output = open(outfn, 'w')
    if not append:
        output.write('eform,slices\n')
    ...
    if scaler_type:
        ...
        energy_scaler=scaler.transform([[float(energy)]])[0][0]
    else:
        energy_scaler=float(energy)
    for i in tqdm(range(gen_iter)):    
        seqs, likelihood, _ = transfer_model.sample(int(batch_size), energy_scaler)
        for i, seq in enumerate(seqs.cpu().numpy()):
            slices = voc.decode(seq)
            #print(slices)
            if check_SLICES(slices,4,dupli_check=False,graph_rank_check=False):
                valid+=1
                output.write(str(energy)+','+slices+'\n')
...
if __name__ == "__main__":
    ...
    sample_slices(args.voc, args.gen_size,args.batch_size, -1,args.save_slices,args.model, append=False, scaler_type= args.scaler)
    sample_slices(args.voc, args.gen_size,args.batch_size, -2,args.save_slices,args.model, append=True, scaler_type= args.scaler)
    sample_slices(args.voc, args.gen_size,args.batch_size, -3,args.save_slices,args.model, append=True, scaler_type= args.scaler)
    sample_slices(args.voc, args.gen_size,args.batch_size, -4,args.save_slices,args.model, append=True, scaler_type= args.scaler)
    sample_slices(args.voc, args.gen_size,args.batch_size, -5,args.save_slices,args.model, append=True, scaler_type= args.scaler)
