...
class MolDataHead(Dataset):
    ...
    """
    def __init__(self, fname, voc, scaler_type=0, load_scaler=False):
        self.voc = voc
        self.smiles = []
        self.energy = []
        ...
        with open(fname, 'r',encoding='utf-8-sig') as f:
            for line in f:
                ...
                self.energy.append(float(line.split(',')[1]))
        if scaler_type:
            self.set_scaler()
        
    def set_scaler(self):
        if self.scaler_type == 1:
            if self.load_scaler:
                print("loading MinMaxScaler.")
                scaler = joblib.load('scaler.pkl')  # 加载scaler
                self.energy_scaler = scaler.transform(np.array(self.energy).reshape(-1, 1))
                self.energy = self.energy_scaler.flatten().tolist()

            else:
                print("Using MinMaxScaler.")
                scaler = MinMaxScaler()
                self.energy_scaler = scaler.fit_transform(np.array(self.energy).reshape(-1, 1))
                #self.energy_df_scaler = scaler.fit_transform(np.array(self.energy_df).reshape(-1, 1))
                #validation_df["target"] = scaler.transform(np.array(validation_df["target"]).reshape(-1, 1))
                self.energy = self.energy_scaler.flatten().tolist()
                joblib.dump(scaler, 'scaler.pkl')  # 保存scaler
                print("Scaler is saved as scaler.pkl")

        elif self.scaler_type == 2:
            if self.load_scaler:
    ...
    def __getitem__(self, i):
        ...
        if encoded is not None:
            return Variable(encoded), Variable(torch.tensor(energy))
    ...
    def collate_fn(cls, arr):
        seqs, energies = zip(*arr)
        """Function to take a list of encoded sequences and turn them into a batch"""
        max_length = max([seq.size(0) for seq in seqs])
        collated_seqs = Variable(torch.zeros(len(seqs), max_length))
        for i, seq in enumerate(seqs):
            collated_seqs[i, :seq.size(0)] = seq
        collated_energies = Variable(torch.tensor(energies).unsqueeze(1))
        return collated_seqs, collated_energies
