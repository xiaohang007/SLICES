...
for i in range(len(grid)):
    ...
    with temporaryWorkingDirectory('job_'+str(i)):
        splitRun_local(filename='../../mp20/cifs_filtered.json',threads=8,skip_header=False)
        show_progress_local()
        collect_csv_filter(output="results.csv",condition=lambda i: len(i.split(','))==12, glob_target="job_*/result.csv", \
        header="'name,opt2_match,opt_match,std_match,opt2_match2,opt_match2,std_match2,opt2_topo2,opt_topo2,std_topo2,natoms,time\n",cleanup=True)
...
with open('../mp20/cifs_filtered.json', 'r') as f:
    cifs=json.load(f)
num=len(cifs)
config = configparser.ConfigParser()
