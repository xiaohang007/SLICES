...
if __name__ == '__main__':
    ...
    prop_condition = args.prop_targets # desired eform

    for i in range(len(prop_condition)):
        print("Target "+str(i+1)+" is : eform"+str(prop_condition[i])+" eV/atom.")
    #for key, value in params.items():
        #print(f"{key}: {value}")
    #print("n_head: ", args.n_head)
    ...
    if prop_condition is not None:
        for c in prop_condition:
            for i in tqdm(range(gen_iter)):
                ...
                y = model.sample(x, params["block_size"], temperature=0.7, do_sample=True, top_k=50, top_p=0.9, prop=p)  
                for gen_mol in y:
                    completion = " ".join([itos[int(i)] for i in gen_mol])
                    completion = completion.replace("<", "").strip()
                    if check_SLICES(completion, 4):
                        all_slices.append({'eform': c, 'SLICES': completion})
                    else:
                        pass
